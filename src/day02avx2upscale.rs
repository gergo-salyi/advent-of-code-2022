...
use std::{
    arch::x86_64::{_mm256_adds_epu16, _mm256_adds_epu8},
    intrinsics::unlikely,
    mem::MaybeUninit,
    ptr::copy_nonoverlapping,
};

const INPUT_BYTES_ORIGINAL: &[u8] = include_bytes!("../res/input02");
pub fn input_bytes() -> Vec<u8> {
    std::fs::read("/home/gergo/projects/advent-of-code-2022/res/input02upscale")
        .unwrap()
}
...
unsafe fn score_avx2(input_slice: &[u8]) -> u64 {
    ...
    let mut acc_u32x8 = _mm256_setzero_si256();
    ...
    for i in 0..clean_chunks {
        ...
        // The u16x16 accumulator fills up every 3640 cycles (65535/18)
        if unlikely(i % 3640 == 3639) {
            // Flush acc_u16x16 into acc_u32x8 and reset acc_u16x16 to zero
            acc_u32x8 = _mm256_add_epi32(
                acc_u32x8,
                hadd_get_u32x8_from_u16x16(acc_u16x16),
            );
            acc_u16x16 = _mm256_setzero_si256();
        }
    }
    ...
    // and acc_u16x16 to acc_u32x8
    acc_u16x16 =
        _mm256_adds_epu16(acc_u16x16, hadd_get_u16x16_from_u8x32(acc_u8x32));

    acc_u32x8 =
        _mm256_add_epi32(acc_u32x8, hadd_get_u32x8_from_u16x16(acc_u16x16));

    // Horizontal sum acc_u32x8 to get the answer
    horizontal_sum_u32x8(acc_u32x8)
}
...
#[inline(always)]
unsafe fn horizontal_sum_u32x8(score: __m256i) -> u64 {
    let a = _mm256_cvtepu32_epi64(_mm256_extracti128_si256::<0>(score));
    let b = _mm256_cvtepu32_epi64(_mm256_extracti128_si256::<1>(score));
    let score = _mm256_add_epi64(a, b);
    let a = _mm256_extracti128_si256::<0>(score);
...
#[inline(always)]
unsafe fn hadd_get_u32x8_from_u16x16(u16x16: __m256i) -> __m256i {
    let a = _mm256_cvtepu16_epi32(_mm256_extracti128_si256::<0>(u16x16));
    let b = _mm256_cvtepu16_epi32(_mm256_extracti128_si256::<1>(u16x16));
...
mod tests {
    ...
    #[test]
    fn test1() {
        assert_eq!(run1(&input_bytes()), 17189 * 40000)
    }

    #[test]
    fn test1old() {
        assert_eq!(run1(INPUT_BYTES_ORIGINAL), 17189)
    }
